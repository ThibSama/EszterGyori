...
items carry a history array or whose envelope lacks the page facts, the mutation
transport adopts the returned current-state booking with no history expected or parsed,
and the conflict reload adopts the authoritative row from the new reference envelope —
the booking beside its own fixed, bounded history page — exactly as before.
