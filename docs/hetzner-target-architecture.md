...
| SQL | `php/migrations/` — ordered, forward-only, individually idempotent files applied by `php/bin/migrate.php` and recorded in `schema_migrations` with their checksums. Since ESZ-112 the read-time guard also refuses row-mutating DML (UPDATE, DELETE, REPLACE, TRUNCATE, RENAME) — no guarded form of them exists that the migrator can verify as repeat-safe. Package 4.1 adds `booking_services`, rule-driven availability, `bookings` and `system_settings`; there is deliberately no future-slot table. Gates `sql:migrations`, `sql:integration`, `sql:rate-limits`, `sql:backup-restore` and `sql:notifications` run against a disposable MySQL 8.4 that the validate runner provisions itself when `ESZTER_TEST_DB_DSN` is absent (ESZ-112) — an external DSN is honoured as-is. |
