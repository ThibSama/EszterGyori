...
const gates = [
  ...
  },
  {
    ...
    proves:
      "ESZ-080/082: the static export, generated contracts, PHP runtime, migrations and production-only locked Composer dependencies form a deterministic archive; only public_html is web-facing, no secret/config or source/test/cache artifact is included, Symfony Mailer and every production operator entry point is present, and Node is build-time only. ESZ-126 adds source identity to the same manifest: packaging requires a clean committed candidate of ThibSama/EszterGyori (staged or unstaged tracked drift refuses before anything is emitted, ignored build outputs are allowed), the manifest carries a Git-derived provenance object with the canonical repository and the exact 40-hex HEAD, and the built-in verifier attests that provenance — directory tree and archived manifest — against the enclosing checkout's identity/HEAD, so a valid artifact can always be tied to the commit that produced it. ESZ-143 closes the one input that provenance could not cover: front/out is ignored by Git, so packaging now deletes it and rebuilds the export from this candidate's tracked source (running the export verifier) before copying a byte, instead of trusting whatever an earlier source state left on disk, and the artifact carries every app/bin command the deployment runbook tells an operator to run.",
  },
  {
    id: "deployment:runbook",
    stage: "5. Build",
    cwd: ".",
    command: ["node", "--test", "scripts/production-artifact-runbook.test.mjs"],
    proves:
      "ESZ-143: the deployment runbook is executable, not prose. The build procedure is parsed out of docs/deployment-runbook.md's marked block and run verbatim in a disposable clean checkout that owns no export, no installed dependency set and no previous artifact — so a documented command that does not exist (the runbook asked for `package:production` and `verify:production-artifact`, neither of which was ever a script) fails here instead of failing an operator. The same run proves the stale-output regression on clean committed source identities: an ignored front/out seeded with bytes no build could emit is rebuilt rather than shipped, and an export from state A carried onto a *different clean committed* state B is absent from B's artifact — the checkouts are asserted clean first, so this cannot pass by re-proving ESZ-126's drift guard. Finally the runbook's own bin/*.php invocations are reconciled with the artifact: every one is packaged and answers its no-network --help from the release, repository tooling stays out, and omitting one is a deterministic verification failure.",
  },
  ...
  },
  {
    ...
    unavailable: sqlDatabaseUnavailable,
    proves:
      "ESZ-070/071/072 against real MySQL, because almost none of it is true on any other engine. A repeated enqueue resolves to the same row and does not reschedule it, while a key reused for a different booking is refused rather than silently ignored. A claim takes a durable lease, charges one attempt and is invisible to a second claim; a job that is not yet due, or whose channel has no transport, is never claimed at all. Delivery succeeds exactly once and `sent` is terminal. Transient failures retry on the frozen 60/120/240/480-second backoff and become terminal on the fifth attempt; a permanent refusal is terminal on the first; a transport that throws anything else is classified as transient and its message never reaches storage or the log. An abandoned lease is recovered a second after it expires and not a second before, without forgiving the attempt it charged, so a job that kills every runner exhausts its budget instead of looping. A runner whose lease expired mid-delivery cannot record a delivery it no longer owns. One run claims at most its batch, so a backlog drains across ticks. A stale reminder is retired before it can be claimed and again after, and never delivered; a non-time-sensitive job is never retired for being old; a disabled channel produces terminal skips so a later re-enable finds nothing pending to burst. No customer name, address, phone, note or database credential appears in any log line, and every key on every line is on the frozen allowlist. Finally, two independent operating-system processes blocked on the same row prove that exactly one claims, exactly one delivers, and the job records exactly one attempt. ESZ-111 makes the persisted transition authoritative for runner reporting, with deterministic real-MySQL lease takeovers between the claim and the guarded outcome write: a delivery the transport really made is never reported `sent` once the lease was lost (sent=0, leasesLost=1, no `notification.sent`, and the rightful owner still records the one `sent`); a transient failure whose lease was lost is neither retried nor failed (retried=0, failed=0, leasesLost=1, no retry or failure log, no backoff or terminal write); a permanent refusal by a runner that lost the row does not fail the job (failed=0, leasesLost=1, and the new owner still delivers); a post-claim stale reminder whose lease was lost is never counted `skipped` (skipped=0, leasesLost=1) and the rightful owner's skip lands exactly once; the fifth transient failure with a valid lease persists terminal `failed` with `attempts_exhausted` and logs `notification.failed`, never `notification.retry`; every ordinary sent/retry/failed/skipped outcome counts once and its log line carries the stored status; and every lease-loss line is allowlisted and carries no `status` — the claim-time `processing` snapshot is never presented as current state — and no customer fact. ESZ-131 makes the lifecycle notifications temporally coherent against real MySQL and the real booking lifecycle: a move supersedes the pending confirmation and any pending earlier move inside its own transaction and a cancellation supersedes both (terminal `skipped` with the frozen `superseded_by_move` / `superseded_by_cancellation` codes), so only the newest applicable move — or the cancellation — remains pending and deliverable; each lifecycle job carries the non-PII booking_history id of its own event, and a job already claimed when the superseding transition committed is re-checked against that ordering before the transport boundary and skipped with the same code, so a delayed cron or a transient retry never renders an obsolete e-mail under current facts (rendered subject and appointment instant are asserted through the real renderer and a recording mailer, not only queue rows); a claimed job whose lease a second runner took over is reported lease-lost by its stale owner — never sent or retried — and only the rightful owner records the terminal skip; reminders keep their catch-up, stale-window and move-rescheduling rules with no lifecycle marker.",
  },
