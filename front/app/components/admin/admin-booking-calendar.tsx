...
import { PRIVACY_BOOKING_MARKER_LABELS } from "../../lib/admin-privacy-requests";

/**
 * ESZ-164 — how a booking is named on the calendar. An anonymised booking
 * has no customer left to name: it is shown as
 * `Cliente anonymisée — rendez-vous maintenu`, never as the stored
 * placeholder, so the frozen placeholder can never read as a person.
 */
function displayName(booking: Pick<AdminBooking, "customerName" | "customerDataErasedAt">): string {
  return booking.customerDataErasedAt === null ? booking.customerName : PRIVACY_BOOKING_MARKER_LABELS.anonymised;
}
import {
  addCivilDays,
  bookingsForDate,
...
function weekAppointmentLabel(item: WeekAppointment, serviceLabel: (key: string | readonly string[]) => string): string {
  ...
  return `${item.startLocal} – ${item.endLocal}, ${displayName(item.booking)}, ${serviceLabel(item.booking.serviceKeys)}, ${state}`;
}
          ...
          */}
          <section className="min-w-0 rounded-3xl border border-warm-200 bg-white p-4 shadow-sm sm:p-6" aria-busy={loading}>
            ...
            {loading ? <p role="status" className="py-16 text-center text-warm-600">Chargement des rendez-vous…</p> : view === "week" ? (
                            ...
                            <div
                              ...
                              {items.map((item) => (
                                <button
                                  ...
                                  <span aria-hidden="true" className="block truncate">{displayName(item.booking)}</span>
                                  {item.span > 1 && (
              ...
              <div className="mt-6">
                ...
                {dayBookings.length === 0 ? <p className="py-16 text-center text-warm-600">Aucun rendez-vous ce jour.</p> : <ul className="mt-5 space-y-3">{dayBookings.map((booking) => <li key={booking.reference}><button type="button" onClick={() => chooseBooking(booking)} className={`flex w-full items-center justify-between gap-4 rounded-2xl border p-4 text-left focus:outline-none focus:ring-2 focus:ring-sage-300 ${booking.state === "cancelled" ? "border-warm-200 bg-warm-50 text-warm-500" : "border-sage-200 bg-sage-50/50"}`}><span><span className="block text-lg font-medium">{formatParisTime(booking.startsAtUtc)} · {displayName(booking)}</span><span className="mt-1 block text-sm">{serviceLabel(booking.serviceKeys)}{booking.processingRestrictedAt !== null ? <span className="ml-2 rounded-full bg-rose-100 px-2 py-0.5 text-xs font-semibold text-rose-800">{PRIVACY_BOOKING_MARKER_LABELS.restricted}</span> : null}</span></span><span className={`rounded-full px-3 py-1 text-xs font-semibold uppercase ${booking.state === "cancelled" ? "bg-warm-200" : "bg-sage-200 text-sage-900"}`}>{booking.state === "cancelled" ? "Annulé" : "Confirmé"}</span></button></li>)}</ul>}
              </div>
            )}
          </section>
          ...
          <aside className="rounded-3xl border border-warm-200 bg-white p-5 shadow-sm sm:p-6" aria-label="Détail du rendez-vous">
            {!selected ? <div className="flex min-h-64 items-center justify-center text-center text-sm text-warm-600">Sélectionnez un rendez-vous dans la vue Jour pour afficher ses détails.</div> : <div>
              <h2 ref={detailHeadingRef} tabIndex={-1} className="font-display text-2xl text-warm-900 focus:outline-none">{displayName(selected)}</h2>
              <p className="mt-1 text-sm text-warm-600">{serviceLabel(selected.serviceKeys)}</p>
              {selected.customerDataErasedAt !== null && <p className="mt-3 rounded-2xl bg-warm-100 p-3 text-sm text-warm-700">Les données personnelles de cette cliente ont été anonymisées ; le rendez-vous est conservé. Aucune coordonnée ne peut être affichée ni modifiée.</p>}
              {selected.processingRestrictedAt !== null && <p className="mt-3 rounded-2xl border border-rose-200 bg-rose-50 p-3 text-sm text-rose-900"><span className="rounded-full bg-rose-100 px-2 py-0.5 text-xs font-semibold uppercase text-rose-800">{PRIVACY_BOOKING_MARKER_LABELS.restricted}</span> Aucun rappel par e-mail ou SMS n’est envoyé pour ce rendez-vous tant que la limitation n’est pas levée depuis la demande RGPD qui l’a posée.</p>}
              <dl className="mt-5 grid gap-4 text-sm"><div><dt className="text-warm-500">Date et heure (Paris)</dt><dd className="mt-1 font-medium capitalize">{formatParisDate(parisLocalDate(selected.startsAtUtc))}, {formatParisTime(selected.startsAtUtc)}</dd></div><div><dt className="text-warm-500">État</dt><dd className="mt-1 font-medium">{selected.state === "cancelled" ? "Annulé" : "Confirmé"}</dd></div>{selected.customerDataErasedAt === null && <div><dt className="text-warm-500">Email</dt><dd className="mt-1 break-all"><a className="underline" href={`mailto:${selected.customerEmail}`}>{selected.customerEmail}</a></dd></div>}{selected.customerDataErasedAt === null && selected.customerPhone && <div><dt className="text-warm-500">Téléphone</dt><dd className="mt-1"><a className="underline" href={`tel:${selected.customerPhone}`}>{selected.customerPhone}</a></dd></div>}{selected.customerDataErasedAt === null && selected.customerNote && <div><dt className="text-warm-500">Note</dt><dd className="mt-1 whitespace-pre-wrap">{selected.customerNote}</dd></div>}</dl>
              {action === "none" && <div className="mt-6 flex flex-wrap gap-2">{selected.customerDataErasedAt === null && <button type="button" onClick={beginContactEdit} className="rounded-full border border-warm-300 px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sage-300">Modifier les coordonnées</button>}{selected.state === "confirmed" && <><button type="button" onClick={beginMove} className="rounded-full bg-warm-900 px-4 py-2 text-sm text-white focus:outline-none focus:ring-2 focus:ring-sage-300">Déplacer</button><button type="button" onClick={() => { setAction("cancel"); setCancelReason(""); requestAnimationFrame(() => actionHeadingRef.current?.focus()); }} className="rounded-full border border-rose-300 px-4 py-2 text-sm text-rose-800 focus:outline-none focus:ring-2 focus:ring-rose-300">Annuler</button></>}</div>}
